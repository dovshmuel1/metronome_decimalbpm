...
public class Metronome {
    ...
    public Metronome(byte[] mainFileBytes, byte[] accentedFileBytes, double bpm, int timeSignature, float volume,
                     int sampleRate) { // DECIMAL BPM MOD
        SAMPLE_RATE = sampleRate;
        audioBpm = bpm; // DECIMAL BPM MOD
        audioVolume = volume;
        audioTimeSignature = timeSignature;
        mainSound = byteArrayToShortArray(mainFileBytes);
        accentedSound = (accentedFileBytes.length == 0) ? mainSound : byteArrayToShortArray(accentedFileBytes);
    ...
    public void play() {
        if (!isPlaying()) {
            updated = true;
            onTick();
            if (eventTickSink != null) eventTickSink.success(0); // Immediate tick
            audioTrack.play();
            startMetronome();
        }
    }
    ...
    public void setAudioFile(byte[] mainFileBytes, byte[] accentedFileBytes) {
        if (mainFileBytes.length > 0) mainSound = byteArrayToShortArray(mainFileBytes);
        if (accentedFileBytes.length > 0) accentedSound = byteArrayToShortArray(accentedFileBytes);
    ...
    public boolean isPlaying() { return audioTrack.getPlayState() == PLAYSTATE_PLAYING; }
    public void enableTickCallback(EventChannel.EventSink _eventTickSink) { eventTickSink = _eventTickSink; }
    ...
    private short[] generateBuffer() {
        currentTick = 0;
        // DECIMAL BPM MOD: round instead of cast
        int framesPerBeat = (int) Math.round(SAMPLE_RATE * 60.0 / audioBpm);
    ...
    void onTick() {
        if (eventTickSink == null) return;

        int framesPerBeat = (int) Math.round(SAMPLE_RATE * 60.0 / audioBpm); // DECIMAL BPM MOD
        ...
        audioTrack.setPlaybackPositionUpdateListener(new AudioTrack.OnPlaybackPositionUpdateListener() {
            ...
            public void onPeriodicNotification(AudioTrack track) {
                if (!updated) {
                    if (audioTimeSignature < 2) currentTick = 0;
                    else {
                        currentTick++;
                        if (currentTick >= audioTimeSignature) currentTick = 0;
                    }
    ...
    private void startMetronome() {
        new Thread(() -> {
            while (isPlaying()) {
                synchronized (mLock) {
                    if (!isPlaying()) return;
                    if (updated) audioBuffer = generateBuffer();
                    else audioTrack.write(audioBuffer, 0, audioBuffer.length);
                }
            }
        }).start();
    }
